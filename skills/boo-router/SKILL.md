...
- Provider strings use Pi/OMP format: `provider/model`. Examples: `litellm/deepseek/deepseek-v4-flash`, `litellm/openrouter/xiaomi/mimo-v2.5`, `litellm/minimax-m3` (cloud via LiteLLM proxy), `llama-swap/<model>` (local), `anthropic/claude-sonnet-4-6`, `openai-codex/gpt-5.4`, `cursor/composer-2.5`, `google-antigravity/gemini-3.1-pro` (Gemini stays native, not litellm). Legacy `openrouter/`, `deepseek/`, `kilo/`, and `opencode/*` strings are still recognized by the router but presets emit `litellm/` routes.
...
- Provider priority: the registry `provider_priority` block adds a per-SOURCE bonus so equivalent models route to the preferred provider. Current order (2026-06): litellm (LiteLLM proxy) > deepseek (legacy direct) > kilo > openrouter > local (llama-swap) > local-edge > gateway-free > subscription. Legacy `opencode-go` remains deprioritized when present. Source is classified from the provider string.
- Cross-provider pools: a role pool may list several cloud models under `litellm/` (e.g. `litellm/openrouter/xiaomi/mimo-v2.5`, `litellm/deepseek/deepseek-v4-flash`). The router picks the highest-scoring candidate and returns the rest as the `fallbacks` chain, so a dead route fails over to the next.
