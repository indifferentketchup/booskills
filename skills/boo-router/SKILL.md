...
- The active preset is `~/.paseo/orchestration-preferences.json` (the file `paseo-preset` copies a named preset onto), not a file under `presets/`. Switch grade pools with `paseo-preset grade-D|grade-C|grade-B|grade-A|grade-S|workhorse`.
- Provider strings use Pi/OMP format: `provider/model`, wrapped in `omp/` for anything OMP resolves itself. DeepSeek, Xiaomi, and MiniMax have native direct-API integrations inside OMP and route as `omp/deepseek/deepseek-v4-flash`, `omp/xiaomi/mimo-v2.5`, `omp/minimax-code/MiniMax-M3` (note MiniMax's capitalized native ids); prefer these native routes over `litellm/` for these three providers when available. (Unverified: an earlier draft of this note claimed OMP's model resolver 404s on `litellm/deepseek/...` and `litellm/openrouter/xiaomi/...`, "confirmed in daemon logs 2026-07-20 to 2026-07-24." A direct search of `~/.omp/logs/` covering that exact window found zero occurrences of either string; routing during that window used `openrouter/`-prefixed models, not `litellm/`-prefixed ones. The 404 claim could not be confirmed and is not repeated here.) Other cloud gateway models without a native OMP integration still go through the proxy: `omp/litellm/openrouter/z-ai/glm-5`, `omp/litellm/moonshot/kimi-k2.6`. Local models: `llama-swap/<model>` (no omp/ wrapper). Native subscription backends stay direct, no wrapper: `anthropic/claude-sonnet-4-6`, `openai-codex/gpt-5.4`, `cursor/composer-2.5`, `google-antigravity/gemini-3.1-pro`. Legacy `openrouter/`, `deepseek/`, `kilo/`, and `opencode/*` strings (without the `omp/` wrapper) are still recognized by the router's `sourceOf` classifier for back-compat, but presets should emit the `omp/`-wrapped forms above.
- The `agents` map uses full `provider/model` strings. The router keys attributes/pricing/quota by the last path segment, so the provider namespace can change without touching the registry.
- Local models use `llama-swap/`; only one is resident at a time, so pass `--resident-local <id>` to earn the no-swap bonus and avoid thrashing.
...
- Provider priority: the registry `provider_priority` block adds a per-SOURCE bonus so equivalent models route to the preferred provider. Current order (2026-07): litellm (LiteLLM proxy) > deepseek / xiaomi / minimax-code (native direct APIs via omp, tied) > kilo > openrouter > local (llama-swap) > local-edge > gateway-free > subscription. Legacy `opencode-go` remains deprioritized when present. `sourceOf` strips a leading `omp/` before classifying, so `omp/deepseek/...` still scores as `deepseek`.
- Cross-provider pools: a role pool may list several cloud models (e.g. `omp/xiaomi/mimo-v2.5`, `omp/deepseek/deepseek-v4-flash`). The router picks the highest-scoring candidate and returns the rest as the `fallbacks` chain, so a dead route fails over to the next.
- On Pi, pass routed `provider/model` strings to `task()` assignments or OMP `modelRoles`; Paseo `create_agent` is optional when Paseo MCP is absent.
