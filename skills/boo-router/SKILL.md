...
- Provider strings use Pi/OMP format: `provider/model`. Examples: `deepseek/deepseek-v4-flash` (DeepSeek direct API), `openrouter/<vendor>/<model>` and `kilo/<vendor>/<model>` (dual-gateway pools), `llama-swap/<model>` (local), `anthropic/claude-sonnet-4-6`, `openai-codex/gpt-5.4`, `cursor/composer-2.5`, `google-antigravity/gemini-3.1-pro`. Legacy `opencode/*` strings are still recognized by the router but are no longer emitted by presets.
...
- Provider priority: the registry `provider_priority` block adds a per-SOURCE bonus so equivalent models route to the preferred provider. Current order (2026-06): deepseek (direct API) > kilo > openrouter > local (llama-swap) > local-edge > gateway-free > subscription. Legacy `opencode-go` remains deprioritized when present. Source is classified from the provider string.
- Cross-provider pools: a role pool may list the same logical model via several providers (e.g. `openrouter/xiaomi/mimo-v2.5` and `kilo/xiaomi/mimo-v2.5`). The router picks the highest-priority source and returns the rest as the `fallbacks` chain, so a dead provider fails over to the next.
