...
- Provider strings use Pi/OMP format: `provider/model`, wrapped in `pi/` (or legacy `omp/`) for anything the agent CLI resolves itself. DeepSeek, Xiaomi, and MiniMax have native direct-API integrations inside Pi and route as `pi/deepseek/deepseek-v4-flash`, `pi/xiaomi/mimo-v2.5`, `pi/minimax/MiniMax-M3` (note MiniMax's capitalized native id and that Pi's subprovider slug is `minimax`, not OMP's legacy `minimax-code`); prefer these native Pi routes over `litellm/` for these three providers when available. Other cloud gateway models without a native Pi integration still go through OMP's litellm proxy (Pi does not yet expose one): `omp/litellm/openrouter/z-ai/glm-5`, `omp/litellm/moonshot/kimi-k2.6`. Local models: `llama-swap/<model>` (no wrapper). Native subscription backends stay direct, no wrapper: `anthropic/claude-sonnet-4-6`, `openai-codex/gpt-5.4`, `cursor/composer-2.5`, `google-antigravity/gemini-3.1-pro`. Legacy `openrouter/`, `deepseek/`, `kilo/`, and `opencode/*` strings (without a `pi/`/`omp/` wrapper) are still recognized by the router's `sourceOf` classifier for back-compat, but presets should emit the `pi/`-wrapped native forms above.
...
- Provider priority: the registry `provider_priority` block adds a per-SOURCE bonus so equivalent models route to the preferred provider. Current order (2026-07): litellm (LiteLLM proxy) > deepseek / xiaomi / minimax-code (native direct APIs via Pi, tied) > kilo > openrouter > local (llama-swap) > local-edge > gateway-free > subscription. Legacy `opencode-go` remains deprioritized when present. `sourceOf` strips a leading `pi/` or `omp/` before classifying, and normalizes Pi's `minimax/` slug onto the same `minimax-code` bucket as OMP's, so `pi/deepseek/...` still scores as `deepseek` and `pi/minimax/...` still scores as `minimax-code`.
- Cross-provider pools: a role pool may list several cloud models (e.g. `pi/xiaomi/mimo-v2.5`, `pi/deepseek/deepseek-v4-flash`). The router picks the highest-scoring candidate and returns the rest as the `fallbacks` chain, so a dead route fails over to the next.
- On Pi, pass routed `provider/model` strings to `task()` assignments; OMP `modelRoles` only apply to legacy OMP sessions. Paseo `create_agent` is optional when Paseo MCP is absent.
