...
**Process (Han /research pattern):** define the decision the research serves (research without a decision is a YAGNI failure; ask once if unclear) → local evidence via `boocontext_explore`/`boocontext_callgraph` when available, direct reads otherwise → fan out `source-researcher` (web-capable, one dispatch per candidate, or per source class when no candidates are named) to gather claims with source URL, publication or commit date, version pin, and primary/secondary judgment → gather any remaining evidence wide-then-deep via the Context7-then-firecrawl-then-`WebFetch` chain, preferring primary sources over blogs → tag every claim with trust class (codebase / web / provided), `Primary?`, `Version/Date`, and corroboration status → conflicting sources recorded both, disagreement named → dispatch `adversarial-validator` against the assembled claims table and draft recommendation before the report is written → recommendation only from claims that pass the corroboration gate (two independent non-derived sources, or a single primary source); single-source (secondary) claims may inform but must be flagged inline.

**Output:** decision statement, recommendation with non-technical axes (license, maintenance signal, bus factor, cost), `## Options considered and rejected`, six-column claims table (claim, source URL, trust class, `Primary?`, `Version/Date`, corroboration status), `## No evidence yet` section when applicable, `## Sources consulted`, "Claims I did not verify."
